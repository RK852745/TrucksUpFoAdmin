...
namespace TrucksUpFoAdmin.Controllers
{
    public class AdminController : Controller
    {
                        ...
                        { 
                            DataList = (from rows in ds.Tables[0].AsEnumerable()
                                        select new Cl_datalistModel
                                        {
                                            ...
                                            entrydatetime = Convert.ToString(rows["entrydatetime"]),
        ...
        [HttpPost]
        public ActionResult DashboardDataExportToExcel(string startdate, string enddate, string foid, string tvisit, string owntype, string Status, int start, int length, string searchValue, int draw)
        {
            start = 0;
            length = int.MaxValue;
            try
            {
                if (UtilityModule.getUserTokenCheck())
                {
                    if (UtilityModule.getParamFromUrl("isadmin").ToUpper() == "Y")
                    {
                        ...
                        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                        {
                            List<Cl_datalistModel> DataList = (from rows in ds.Tables[0].AsEnumerable()
                                                               select new Cl_datalistModel
                                                               {
                                                                   FovmId = Convert.ToInt64(rows["FovmId"]),
                                                                   Name = Convert.ToString(rows["Name"]),
            ...
            return Json(new { draw, recordsTotal = 0, recordsFiltered = 0, data = new List<Cl_datalistModel>() });
        }
