...
namespace TrucksUpFoAdmin.Controllers
{
    public class ReportsController : Controller
    {
        ...
        [HttpPost]
        public ActionResult GetVisitsDataAndExportToExcel(string startdate, string enddate, string start, string length)
        {
            start = Convert.ToString("0");
            length = Convert.ToString(int.MaxValue);
            try
            {
                if (UtilityModule.getUserTokenCheck())
                {
                    if (UtilityModule.getParamFromUrl("isadmin").ToUpper() == "Y")
                    {
                        ...
                        List<StickerDataListt> DataList = new List<StickerDataListt>();

                        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                        {
                            DataList = (from rows in ds.Tables[0].AsEnumerable()
                                        select new StickerDataListt
                                        {
                                            fullname = Convert.ToString(rows["FullName"]),
                                            drivernumber = Convert.ToString(rows["DriverNumber"]),
                                            operatornumber = Convert.ToString(rows["OperatorNumber"]),
                                            ...
                                            vehiclesize = Convert.ToString(rows["VehicleSize"]),
                                            vehiclecapacity = Convert.ToString(rows["VehicleCapacity"]),
                                            vehiclenumber = Convert.ToString(rows["VechileNumber"]),
                                            verifiedstatus = Convert.ToString(rows["VerifiedStatus"]),
                                            verifiedby = Convert.ToString(rows["VerifiedBy"]),
                                            activeflag = Convert.ToString(rows["ActiveFlag"]),
                                            createdby = Convert.ToString(rows["CreatedBy"]),
                            ...
                            using (var excelPackage = new ExcelPackage())
                            {
                                ...
                                ws.Cells["A2"].LoadFromText("FullName,DriverNumber,OperatorNumber,Downloads,StickerImage,StickerSize,LaneFrom,LaneTo,VehicleType,VehicleSize,VehicleCapacity,VechileNumber,VerifiedStatus,VerifiedBy,ActiveFlag,CreatedBy,CreatedDate,ModifiedBy,ModifiedDate,DoneBy");

                                // Load data starting from the second row
                                ws.Cells["A3"].LoadFromCollection(DataList, false);
            ...
            // Return an empty file in case of error or no data
