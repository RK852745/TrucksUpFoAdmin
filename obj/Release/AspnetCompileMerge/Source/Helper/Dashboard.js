...
$(function () {
    // Initialize the default start and end dates to today
    var start = moment();
    var end = moment();
                ...
                $("#Datatable1").DataTable(
                    {
                        ...
                        columns: [
                            ...
                            {
                                render: function (data, type, row) {
                                    return "<div>" + row.entrydatetime + "</div>";
...
function GetFieldOfficers() {
    ...
    if (JsonResult != "") {
        ...
        for (var i = 0; ParseResult.length > i; i++) {
            ...
            list += "<li><div executiveid='" + ParseResult[i].executiveid + "' value='" + ParseResult[i].userid + "' class='foclass'>" + ParseResult[i].name + "<i class='fa fa-check-circle tickcirlce d-none'></i></div></li>";
        }
        ...
        $("#appendfieldOfficerList").empty();
        $("#appendfieldOfficerList").append(searchInput + selectlist);
        $("#folists").empty();
        $("#folists").append(searchInput + selectlist);

        $("#multiselectmodalapppendlist").empty();
        $("#multiselectmodalapppendlist").append(searchInput + list);
...
function Getdata() {
    ...
    var date = $("#reportrange span").text();
    date = date.split("-");
    var startdate = date[0].trim();
    var enddate = date[1].trim();
    var foid = "";
    var tvisit = "";
    var owntype = "";
    var Status = "";
    ...
    $.ajax({
        type: "POST",
        url: "../Admin/DashboardDataExportToExcel",
        //url: "/TrucksupfoAdmin/Admin/DashboardDataExportToExcel",
        data: {
            startdate: startdate,
            enddate: enddate,
            foid: foid,
        ...
        success: function (data) {
            ...
            if (window.navigator.msSaveOrOpenBlob) {
                ...
                window.navigator.msSaveOrOpenBlob(data, 'Dashbord.xlsx');
            } else {
